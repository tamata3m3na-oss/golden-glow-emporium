...
const VerifyPhone = ({
  ...
  // Validate activation code (6 digits)
  const isCodeValid = activationCode.length === 6 && /^\d{6}$/.test(activationCode);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col px-6 py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        {/* Tamara Logo */}
        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="30" fill="none" viewBox="0 0 44 19" style={{ color: '#000000' }}>
        ...
        {/* Right side - Language and Back */}
        <div className="flex items-center gap-2.5">
          <span className="cursor-pointer text-sm" style={{ color: '#555' }}>English</span>
          <button onClick={onBack} className="cursor-pointer text-2xl" style={{ color: '#444', background: 'none', border: 'none' }}>&larr;</button>
        </div>
      </div>
      ...
      <div className="max-w-sm mx-auto w-full flex-1 flex flex-col">
        {/* Title */}
        <h2 className="text-2xl font-bold mb-2 text-right">أدخل رمز التحقق</h2>
        <p className="text-gray-600 text-sm mb-1 text-right">{phoneNumber}</p>
        <button
          onClick={onBack}
          className="text-gray-500 text-sm mb-6 text-right hover:underline"
          style={{ background: 'none', border: 'none', cursor: 'pointer' }}
        >
          تغيير رقمك؟
        </button>

        <p className="text-gray-600 text-sm mb-8 text-right">
          لقد أرسلنا رمز التحقق عبر الرسائل القصيرة
        </p>

        <div className="space-y-6">
          {/* Code Input */}
          <div className="text-right">
            <label className="text-sm font-medium block mb-2 text-gray-700">رمز التحقق</label>
            <input
              value={activationCode}
              onChange={e => {
                const value = toEnglishNumbers(e.target.value.replace(/\D/g, ''));
                setActivationCode(value.slice(0, 6));
                onClearError();
              }}
            ...
            />
            {!isCodeValid && activationCode.length > 0 && (
              <p className="text-red-500 text-xs mt-1">يرجى إدخال رمز التحقق المكون من 6 أرقام</p>
            )}
          </div>

          {codeError && <p className="text-red-500 text-sm text-right">{codeError}</p>}

          <p className="text-gray-600 text-sm text-right">
            {resendTimer > 0 ? `إعادة الإرسال خلال ${formatTimer(resendTimer)}` : (
              <button 
                onClick={() => {}} 
          ...
          {/* Terms Checkbox */}
          <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer text-right">
            <input
              type="checkbox"
              checked={agreedTerms}
            ...
            />
            أوافق على <a href="https://tamara.co/terms" target="_blank" rel="noopener noreferrer" className="text-gray-900 hover:underline">شروط وأحكام تمارا</a>
          </label>
          ...
          <Button
            ...
            style={{
              backgroundColor: isCodeValid && agreedTerms && !isVerifyingCode ? '#000000' : '#dddddd',
              borderRadius: '30px',
              cursor: isCodeValid && agreedTerms && !isVerifyingCode ? 'pointer' : 'not-allowed'
            }}
            disabled={!isCodeValid || !agreedTerms || isVerifyingCode}
          >
            {isVerifyingCode ? (
              <>
